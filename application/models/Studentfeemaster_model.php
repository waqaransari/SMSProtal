...
class Studentfeemaster_model extends CI_Model {
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null, $part_id = null) {
        ...
        if ($part_id != null) {
            $sql .= " AND `student_session`.`part_id` =" . $this->db->escape($part_id);
        }
        if ($category != null) {
    ...
    public function getFeeBetweenDate($start_date, $end_date) {
        ...
        $this->db->join('batch_class', 'batch_class.id= student_session.part_id');
    ...
    public function getFeeByInvoice($invoice_id, $sub_invoice_id) {
        $this->db->select('`student_fees_deposite`.*,students.firstname,student_session.part_id,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,student_fees_master.student_session_id')->from('student_fees_deposite');
        $this->db->join('fee_groups_feetype', 'fee_groups_feetype.id = student_fees_deposite.fee_groups_feetype_id');
