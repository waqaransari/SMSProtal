...
class Classsection_model extends CI_Model {
    ...
    public function update($data) {
        ...
        if (isset($data['id'])) {
            ...
            $parts = $this->db->get_where("batch_parts",array('class_id' => $data['id']))->result_array();
            if(empty($parts)){
                ...
                if(!empty($parts)){
                    foreach($parts as $part){
                        $batch_part[] = array(
                                "class_id" => $data['id'],
                                "part_id" => $part["id"],
                                "session_id" => $this->session->userdata("session_array")["session_id"]
                        );
                    }
                    $this->db->insert_batch('batch_parts', $batch_part);  
                }
            }
        }
    }
    ...
    public function add($data, $sections) {
        ...
        if (isset($data['id'])) {
            ...
            $this->db->update('classes', $data);
            $class_id = $data['id'];
            $parts = $this->db->get_where("batch_parts",array('class_id' => $class_id))->result_array();
            if(empty($parts)){
                $parts = $this->db->get_where("batch_class")->result_array();
                $batch_part = array();
                if(!empty($parts)){
