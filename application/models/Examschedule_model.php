...
class Examschedule_model extends CI_Model {
    ...
    public function getDetailbyClsandSection($class_id, $section_id, $exam_id, $part_id = null) {
        $sqlquery = "";
        if($part_id != null){
            $sqlquery = " and exam_schedules.part_id=" . $this->db->escape($part_id);
        }
        $query = $this->db->query("SELECT exam_schedules.*,subjects.name,subjects.type FROM exam_schedules,teacher_subjects,exams,class_sections,subjects WHERE exam_schedules.teacher_subject_id = teacher_subjects.id and exam_schedules.exam_id =exams.id and class_sections.id =teacher_subjects.class_section_id and teacher_subjects.subject_id=subjects.id and class_sections.class_id =" . $this->db->escape($class_id) . " and class_sections.section_id=" . $this->db->escape($section_id) . " and exam_id =" . $this->db->escape($exam_id) . " and exam_schedules.session_id=" . $this->db->escape($this->current_session).$sqlquery);
        return $query->result_array();
    }

    public function getExamByClassandSection($class_id, $section_id, $part_id = null) {
        $Qsql = "";
        if($part_id != null){
            $Qsql = " where exam_schedules.part_id =" . $this->db->escape($part_id);
        }
