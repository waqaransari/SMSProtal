...
class Teachersubject_model extends CI_Model {
    ...
    public function getDetailByclassAndSection($class_section_id,$part_id = null) {
        $this->db->select()->from('teacher_subjects');
        $this->db->where('class_section_id', $class_section_id);
    ...
    public function getDetailbyClsandSection($class_id, $section_id, $exam_id, $part_id = null) {
        $Qsql = "";
        $cSqL = "";
        if($part_id != null){
            //$sqljoin = " INNER JOIN batch_parts ON class_sections.class_id= batch_parts.class_id ";
            $Qsql = " and exam_schedules.part_id =" . $this->db->escape($part_id);
            $cSqL = " and teacher_subjects.part_id =" .$this->db->escape($part_id);
        }
           $query = $this->db->query("SELECT teacher_subjects.*,exam_schedules.date_of_exam,exam_schedules.start_to,exam_schedules.end_from,exam_schedules.room_no,exam_schedules.full_marks,exam_schedules.passing_marks,subjects.name,
            subjects.type FROM `teacher_subjects` LEFT JOIN `exam_schedules` ON exam_schedules.teacher_subject_id=teacher_subjects.id AND exam_schedules.exam_id = " . $this->db->escape($exam_id) .$Qsql. "  INNER JOIN subjects
            ON teacher_subjects.subject_id = subjects.id INNER JOIN class_sections
            ON teacher_subjects.class_section_id = class_sections.id  WHERE class_sections.class_id =" . $this->db->escape($class_id) . " and class_sections.section_id=" . $this->db->escape($section_id).$cSqL);
            return $query->result_array();
    }
