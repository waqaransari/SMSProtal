...
class Stuattendence extends Admin_Controller {
    ...
    function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $resultlist = $this->stuattendence_model->searchAttendenceClassSection($class, $section, date('Y-m-d', $this->customlib->datetostrtotime($date)), $part_id);
    ...
    function attendencereport() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $attendencetypes = $this->attendencetype_model->get();
            $data['attendencetypeslist'] = $attendencetypes;
            $resultlist = $this->stuattendence_model->searchAttendenceClassSectionPrepare($class, $section, date('Y-m-d', $this->customlib->datetostrtotime($date)), $part_id);
            $data['resultlist'] = $resultlist;
            $this->load->view('layout/header', $data);
            $this->load->view('admin/stuattendence/attendencereport', $data);
            $this->load->view('layout/footer', $data);
    ...
    function classattendencereport() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $studentlist = $this->student_model->searchByClassSection($class, $section, $part_id);
            ...
            for ($i = 1; $i <= $num_of_days; $i++) {
                ...
                $res = $this->stuattendence_model->searchAttendenceClassSection($class, $section, $att_date, $part_id);
                $student_result = $res;
                $s = array();
