...
function getBatchClass($class,$type = null,$edit = ""){
    $CI = get_instance();
    $parts = $CI->db->get_where("batch_class")->result_array();
    $id_n_name = "part_id";
    if($type == "promote"){
        $id_n_name = "part_promote_id";
    }
    $error = '<span class="text-danger">'.form_error('part_id').'</span>';
    $selectpicker ='<label for="exampleInputEmail1">'.$CI->lang->line('part').'</label>
                <select  id="'.$id_n_name.'" name="'.$id_n_name.'" class="form-control">
                <option value="">'.$CI->lang->line('select').'</option>';
               
                foreach ($parts as $part) {
                    $select = "";
                    if($edit != ""){
                        $select = (set_value('part_id', $edit) == $part['id']) ? "selected=selected" : ""; 
                    }else{
                        $select = set_value('part_id');
                    }
                    
                    $selectpicker .= '<option value="'.$part['id'].'" '.$select.' >'.$part["name"].'</option>';
                  
                }
   if($type == "assignTec"){
    $error = '<span class="part_id_error text-danger"></span>';
   }             
    $selectpicker .= '</select>'.$error;            
    if($type != null && $type == "classes"){
        $html = '<div class="form-group">'.$selectpicker.'</div>';
    }else{
        $html = '<div class="'.$class.'"><div class="form-group">'.$selectpicker.'</div></div>';
    }
...
function GetPartName($id,$column){
    $CI = get_instance();
    $parts = $CI->db->select($column)->where("id",$id)->get_where("batch_class")->row_array();
    return $parts[$column];
}
