...
class Textlocalsms
{
	 ...
	 * @param $hash
	 */
	function __construct($array, $apiKey = false)
	{
	...
	private function _sendRequestCurl($command, $params)
	{
		...
		if ($rawResponse === false) {
			throw new Exception('Failed to connect to the Super Solutions Service: ' . $error);
		} elseif ($httpCode != 200) {
			throw new Exception('Bad response from the Solutions Service : HTTP code ' . $httpCode);
		}
